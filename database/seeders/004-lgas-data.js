...
module.exports = {
  up: async (queryInterface) => {
    ...
    const nigeriaResult = await queryInterface.sequelize.query(
      `SELECT id FROM countries WHERE name = 'Nigeria' LIMIT 1`,
      { type: queryInterface.sequelize.QueryTypes.SELECT }
    );

    if (!nigeriaResult || nigeriaResult.length === 0) {
      console.log('⚠️  Nigeria not found. Please run countries seeders first.');
      return;
    }

    const nigeriaId = nigeriaResult[0].id;

    // Get all states for Nigeria
    const statesResult = await queryInterface.sequelize.query(
      `SELECT id, name FROM states WHERE countryId = '${nigeriaId}' ORDER BY name`,
      { type: queryInterface.sequelize.QueryTypes.SELECT }
    );

    if (!statesResult || statesResult.length === 0) {
      console.log('⚠️  No states found for Nigeria. Please run states seeders first.');
      return;
    }

    // Create a map of state names to IDs
    const stateMap = {};
    statesResult.forEach(state => {
      stateMap[state.name] = state.id;
    });

    // All 774 LGAs in Nigeria organized by state
    const lgasByState = {
      ...
      'Kebbi': ['Aleiro', 'Arewa Dandi', 'Argungu', 'Augie', 'Bagudo', 'Bunza', 'Dandi', 'Fakai', 'Gwandu', 'Jega', 'Kalgo', 'Koko/Besse', 'Maiyama', 'Ngaski', 'Sakaba', 'Shanga', 'Suru', 'Danko/Wasagu', 'Yauri', 'Zuru'],
    ...
    for (const [stateName, lgas] of Object.entries(lgasByState)) {
      const stateId = stateMap[stateName];
      if (!stateId) {
        console.log(`⚠️  State "${stateName}" not found, skipping LGAs`);
        continue;
      }

      for (const lgaName of lgas) {
        // Check if LGA already exists
        const existingLga = await queryInterface.sequelize.query(
          `SELECT id FROM lgas WHERE name = '${lgaName.replace(/'/g, "''")}' AND stateId = '${stateId}' LIMIT 1`,
          { type: queryInterface.sequelize.QueryTypes.SELECT }
        );

        if (!existingLga || existingLga.length === 0) {
          lgasToInsert.push({
            id: randomUUID(),
            name: lgaName,
            stateId: stateId,
    ...
    if (lgasToInsert.length > 0) {
      ...
      const batchSize = 100;
      for (let i = 0; i < lgasToInsert.length; i += batchSize) {
        const batch = lgasToInsert.slice(i, i + batchSize);
        await queryInterface.bulkInsert('lgas', batch);
      }
      console.log(`✅ Inserted ${lgasToInsert.length} LGAs`);
    } else {
      console.log('ℹ️  All LGAs already exist');
    }
  },
