"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ReferalUserRepository = void 0;
const common_1 = require("@nestjs/common");
const sequelize_1 = require("@nestjs/sequelize");
const referal_user_entity_1 = require("../entities/referal-user.entity");
let ReferalUserRepository = class ReferalUserRepository {
    referalUserModel;
    constructor(referalUserModel) {
        this.referalUserModel = referalUserModel;
    }
    async create(data) {
        return await this.referalUserModel.create(data);
    }
    async findById(id) {
        return await this.referalUserModel.findByPk(id);
    }
    async findByUserId(userId) {
        return await this.referalUserModel.findAll({
            where: { userId },
            order: [['createdAt', 'DESC']],
        });
    }
    async findByReferredUserId(referredUserId) {
        return await this.referalUserModel.findOne({
            where: { referredUserId },
        });
    }
    async findByReferalCode(referalCode) {
        return await this.referalUserModel.findAll({
            where: { referalCode },
            order: [['createdAt', 'DESC']],
        });
    }
    async update(id, data) {
        return await this.referalUserModel.update(data, {
            where: { id },
            returning: true,
        });
    }
    async markFirstTripCompleted(id) {
        return await this.update(id, { hasCompletedFirstTrip: true });
    }
    async delete(id) {
        return await this.referalUserModel.destroy({
            where: { id },
        });
    }
};
exports.ReferalUserRepository = ReferalUserRepository;
exports.ReferalUserRepository = ReferalUserRepository = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, sequelize_1.InjectModel)(referal_user_entity_1.ReferalUser)),
    __metadata("design:paramtypes", [Object])
], ReferalUserRepository);
//# sourceMappingURL=referal-user.repository.js.map