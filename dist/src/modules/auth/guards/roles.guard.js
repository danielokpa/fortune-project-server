...
let RolesGuard = RolesGuard_1 = class RolesGuard {
    ...
    canActivate(context) {
        const requiredRoles = this.reflector.getAllAndOverride(roles_decorator_1.USERTYPES_KEY, [context.getHandler(), context.getClass()]);
        if (!requiredRoles) {
            return true;
        }
        const { user } = context.switchToHttp().getRequest();
        this.logger.debug(`User role: ${user?.userType}, User orgRole: ${user?.userType}`);
        if (!user?.userType) {
