...
export class SmsEventListener {
  ...
  async handleSignUpOtpEvent(event: SignUpOtpSmsEvent) {
    try {
      ...
      this.logger.log(
        `Signup OTP SMS sent successfully to ${event.phoneNumber}`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to send signup OTP SMS to ${event.phoneNumber}:`,
        error,
      );
    }
  }
  ...
  async handleForgotPasswordOtpEvent(event: ForgotPasswordOtpSmsEvent) {
    try {
      this.logger.log(
        `Sending forgot password OTP SMS to ${event.phoneNumber}`,
      );

      await this.smsService.sendOtp(event.phoneNumber, event.otpCode);

      this.logger.log(
        `Forgot password OTP SMS sent successfully to ${event.phoneNumber}`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to send forgot password OTP SMS to ${event.phoneNumber}:`,
        error,
      );
    }
  }
