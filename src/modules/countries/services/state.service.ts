...
export class StateService {
  ...
  async findAll(params?: Prisma.StateFindManyArgs): Promise<State[]> {
  ...
  async update(id: string, stateData: Prisma.StateUpdateInput): Promise<State> {
    ...
    if (!updatedState) {
      throw new NotFoundException('State not found for update');
    }
  ...
  async delete(id: string): Promise<boolean> {
    ...
    if (!deleted) {
      throw new BadRequestException('Failed to delete state');
    }
